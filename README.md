...
### 5. 중복 발언 제거
- `duplicate_remover.py`의 `DuplicateRemover` 클래스가 담당합니다.
- 형태소 분석(stanza)을 통한 정규화로 유사한 발언을 정확하게 감지합니다.
- 중복 판단 룰 (우선순위 순):
  1. **룰 3: 부분 포함 관계 (최우선)** - A가 A'의 부분집합인 경우 (A ⊂ A') → A 제거
     - 예: "안녕하세요" ⊂ "안녕하세요 반갑습니다" → "안녕하세요" 제거
     - 정규화된 문장으로 비교하며, A의 단어 중 70% 이상이 A'에 포함되면 부분집합으로 판단
  2. **룰 1: 유사도 체크** - 두 발언의 유사도가 50% 이상인 경우 → 동일 처리
  3. **룰 2: 개수 기반 우선순위** - 개수가 적은 행에서 제거, 많은 행 유지
     - 단일 문장인 행은 복수 문장을 가진 행보다 우선 제거 대상
- 성능 최적화: 위로 최대 30개 엔트리만 비교하여 처리 속도 향상
...
- **`duplicate_remover.py`**: 중복 발언 제거 (형태소 분석 기반 정규화 및 중복 판단)
- **`data_merger.py`**: 데이터 병합 처리
