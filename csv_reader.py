...
class CSVReader:
    ...
    def extract_text_from_csv(self, csv_file, progress_tracker):
        ...
        progress_tracker.initialize_tqdm(
            total_rows, "[4단계 중 1단계] 파일 불러오기 및 큰따옴표 발언 추출 중"
        )
        ...
        try:
            for i, row in data.iterrows():
                ...
                for sentence in sentences:
                    _, clean_sentence = text_processor.extract_and_clean_quotes(
                        sentence
                    )
                    ...
                    if len(sentences) == 1:
                        ...
                        for name in candidate_speakers:
                            if text_processor.is_valid_speaker_by_josa(
                                name, clean_sentence
                            ):
                        ...
                        if speakers:
                            add_flag = any(
                                speaker.startswith(row["이름"][0])
                                or row["이름"] in speaker
                                for speaker in speakers
                            )
                            for speaker in speakers:
                                if (
                                    len(speaker) == 3 and len(row["이름"]) == 3
                                ) and speaker != row["이름"]:
                                    add_flag = False
                            if not add_flag:
                    ...
                    if not any(
                        self.validator.is_empty(current_data[field])
